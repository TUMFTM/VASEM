function [vehicle, options, Msg]=Startfile_AuVeCoDe(Parameters,app)
%-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
%                               START FILE                                %
%-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
...
% Manipulated for VASEM by: Ferdinand Schockenhoff and Marc Raudszus (05/2022)
%-------------
...
%%---------------------------Single vehicle development--------------------
    if Parameters.input.single_vehicle==1
        ...
        if vehicle.feasible==1 % vehicle is feasible
            ...
            figure('Name','Vehicle Package','NumberTitle','off','visible','off');
            clf
            ax1 = axes;
            ...
            %----------------Edit by Ferdinand Schockenhoff & Marc Raudszus------------------          
            filename_3D = sprintf('11_Vehicle Concept\\temp\\01_3D_Model_Figures\\VehicleConcept3DModel_%0.0f',app.i_feasible);
            filename_vehicle = sprintf('11_Vehicle Concept\\temp\\01_3D_Model_Figures\\Vehicle_%0.0f_FullData.mat',app.i_feasible);
            saveas(ax1, filename_3D);
            save(filename_vehicle,'vehicle');
            Msg = 'null';
            %----------------Edit by Ferdinand Schockenhoff & Marc Raudszus------------------ 
